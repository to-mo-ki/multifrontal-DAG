...
contains
  subroutine analyze_phase(origin_ccs, max_zero)
    ...
    integer, pointer, contiguous :: first_node(:), last_node(:)
    ...
    ! finding supernode
    call finding_supernode(tree_child, postordering_ccs, tree_child_supernode, ccs_supernode, first_node)
    ! column count
  ...
  subroutine finding_supernode(tree_child_node, ccs_node, tree_child_supernode, ccs_supernode, first_node)
    ...
    integer, pointer, contiguous, intent(out) :: first_node(:)
    ...
    first_node => search_first_node_in_supernode(isleaf, tree_child_node)
    ccs_supernode = create_supernodal_ccs(first_node, ccs_node)
    num_child_supernode => create_supernodal_tree(first_node, tree_child_node)
