program sparse_add_subroutines_test
  ...
  factors => create_factors(node_data, node_set, local_index, 3)
  node_data => create_node_data([4,7,3], [7,3,0], 3)
  ...
  rh%get_work_ptr(1,3) = 1d0
  rh%get_work_ptr(1,4) = 1d0
  rh%get_array_ptr(2,1) = 1d0
  rh%get_array_ptr(2,2) = 1d0
  ...
  call add_test("node=1, index=1", rh%get_array_ptr(2, 1), [1d0,1d0,0d0])
  call sparse_add(factors, rh, block_local_index, 2, 1, 2)
  call add_test("node=1, index=2", rh%get_array_ptr(2, 2), [0d0,1d0,1d0])
  call sparse_add(factors, rh, block_local_index, 3, 1, 2)
  call add_test("node=1, index=3", rh%get_array_ptr(2, 2), [0d0,0d0,0d0])
  call sparse_add(factors, rh, block_local_index, 4, 1, 2)
  call add_test("node=1, index=4", rh%get_array_ptr(2, 3), [1d0,0d0,1d0])
  call sparse_add(factors, rh, block_local_index, 5, 1, 2)
  call add_test("node=1, index=5", rh%get_array_ptr(2, 3), [1d0,0d0,0d0])
  call sparse_add(factors, rh, block_local_index, 6, 1, 2)
  call add_test("node=1, index=6", rh%get_array_ptr(2, 4), [0d0])
