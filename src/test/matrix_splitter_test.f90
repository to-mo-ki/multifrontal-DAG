...
contains
  subroutine diag_test()
    double precision :: from(7, 7), to_left(3*7), to_right(4*4)
    ...
    do i=1, 7
      do j=1, i
        from(j, i) = dble(i*10+j)
      enddo
    enddo
    call split_tri_matrix(from, to_left, to_right, 3, 4, 7)
    ...
    call assert_equal_partial_array("diag:left", to_left, pos_left, 18, check_left)
    
    pos_right = (/1, 5, 6, 9, 10, 11, 13, 14, 15, 16/)
    check_right = (/44d0, 54d0, 55d0, 64d0, 65d0, 66d0, 74d0, 75d0, 76d0, 77d0/)
    call assert_equal_partial_array("diag:right", to_right, pos_right, 10, check_right)

  end subroutine

  subroutine rect_test()
    double precision :: from(5, 4), to_left(2*4), to_right(3*4)
    ...
    call split_rect_matrix(from, to_left, to_right, 2, 3, 4, 5)
    check_left = (/11d0, 12d0, 21d0, 22d0, 31d0, 32d0, 41d0, 42d0/)
    call assert_equal("rect:left", to_left, check_left)
    check_right = (/13d0, 14d0, 15d0, 23d0, 24d0, 25d0, 33d0, 34d0, 35d0, 43d0, 44d0, 45d0/)
    call assert_equal("rect:right", to_right, check_right)
