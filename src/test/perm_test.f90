program perm_test
  ...
  call assert_equal_array("permtate_int", out_int, n, (/1, 7, 2, 4, 3, 5, 6, 8, 9/))

  in_DP = (/1d0, 2d0, 3d0, 4d0, 5d0, 6d0, 7d0, 8d0, 9d0/)
  call permutate(perm, in_DP, out_DP)
  call assert_equal_array("permtate_DP", out_DP, n, (/1d0, 7d0, 2d0, 4d0, 3d0, 5d0, 6d0, 8d0, 9d0/))
  
  call inverse_permutate(perm, in_DP, out_DP)
  call assert_equal_array("inverse_permutate", out_DP, n, (/1d0, 3d0, 5d0, 4d0, 6d0, 7d0, 2d0, 8d0, 9d0/))

  call set_iperm(perm, iperm)
  call assert_equal_array("set_iperm", iperm, n, (/1, 3, 5, 4, 6, 7, 2, 8, 9/))
